...
class LayerCapture:
    ...
    @staticmethod
    def _storage_ptrs(tensors: list[torch.Tensor]) -> list[int]:
        return sorted({int(t.data_ptr()) for t in tensors if torch.is_tensor(t)})
    ...
    def configure_state_capture(self, *, model: object, required: bool) -> None:
        ...
        for name in fa_names:
            fa_ptrs.extend(self._storage_ptrs(self._cache_tensors(getattr(named.get(name), "kv_cache", None))))
        ...
        gdn_ptrs = [ptr for module in gdn_modules.values()
                    for ptr in self._storage_ptrs(self._cache_tensors(getattr(module, "kv_cache", None)))]
        self._gdn_modules = gdn_modules
        self.state_capture.update({
            "enabled": bool(gdn_modules),
            "gdn_layers": sorted(gdn_modules),
            "fa_cache_ptrs": sorted(set(fa_ptrs)),
            "gdn_cache_ptrs": sorted(set(gdn_ptrs)),
            "alias_free": not (set(fa_ptrs) & set(gdn_ptrs)),
        })
        if self.state_capture["required"] and not gdn_modules:
            raise RuntimeError("校准: 未找到带 bound kv_cache 的 GDN 层，拒绝声称 GDN 状态已观测")
        if self.state_capture["required"] and not self.state_capture["alias_free"]:
            raise RuntimeError("校准: GDN state 与 FA KV cache 共享 storage，拒绝继续")
