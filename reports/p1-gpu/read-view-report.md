...
- 三种计数口径不同，勿混用：**30 case-runs**（10 条配置 × 3 种子）、**57 条 step/row 数值测量**
  （784 表 54 条 + 16 对照 3 条）、**972 条判据**（判据数**不是**独立实验样本数）。
- 784 普通分布用例**全种子合并**后的实际最大绝对误差 = **0.0006069093942642212**
  （`trajectory_resident` seed 2 step5，该点 rms = 8.564439771122093e-05）；
  784 最大 rms = **8.738009922283544e-05**（seed 2 step3）。
- 表中 `tail_b = 0.00058` **只是 seed 0 的逐用例值**，不代表所有种子/轨迹步的最大值；此前文字里把它当成
  合并上界的说法有误导，**已更正**为上面的精确值。
- `block16_control`（非 784 对照）最大 = 0.0036176443099975586。
- 上述合并值与本地主代理独立复跑**逐位一致**（同夹具、确定性）。
