...
- **更正后的设计（待实现，仅 CPU/设计）**：
  - **用真实 renderer 构造更长的 prompt**（仍 ≤8192），使 **local_window 起点前存在完整的已写块**：
    例如让上下文档数使 local_window 只覆盖末尾 1–2 块，则**更早的、已被写入的块整体落在可见集合之外** ⇒ 这才是"确实排除至少一个已写块"的实例；
    `focus` 臂选择其中一个较早 segment 作 refs：**refs 所在块可见，其余更早块仍被排除**（与合同一致）。
  - **各模式可见集合由 CPU 独立计算**（用合同实现 `readview`/`gpuoracle` 作参考侧口径，**候选侧不得复用**任何筛选 helper/压缩块表），并把"写过的块集合"与"可见块集合"的差集作为**结构判据**。
  - **尾块跨界**单独构造：让被排除/被包含的边界落在**已写 KV 的块边界**上（不是"span 在块内结束"）。
