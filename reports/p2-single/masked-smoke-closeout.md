...
- 入口、精确结果、原始证据、未验证边界见 `masked-smoke-report.md`；复跑工具为 `tools/p2-diagnostic-run.py`（`--config` 选臂，默认 `configs/p2-masked-smoke/diagnostic.json`），离线观察工具为 `tools/p2-masked-smoke-audit.py`，均要求新输出路径。
