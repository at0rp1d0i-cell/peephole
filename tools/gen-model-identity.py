...
def main() -> int:
    ...
    add(f"- 非 LFS 文件另有 git blob id 校验（13/13 通过，见 evidence-index）")
    add("")
    add("## 文件清单（大小 + sha256）")
    add("")
    ...
    add("")
    add(f"- `enable_thinking=false`：36 token，token id 序列与本地渲染**完全一致**（`server_matches_local_off_ids=true`）")
    add(f"- `enable_thinking=true`：72 token（注入 reasoning 指令 + 未闭合 `<think>`）")
    add("")
